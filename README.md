...
We also learned the importance of keeping dependencies up to date. When we ran npm audit, we discovered several vulnerabilities in our project, including three high-severity issues. To address these, we used npm audit fix, which successfully reduced both the number and severity of the vulnerabilities. However, we were not fully satisfied, as two low severity vulnerabilities remained. We then attempted to resolve all issues by using a forced fix. After doing so, we noticed that this introduced additional high severity vulnerabilities. Once those were resolved, our GitHub Actions security check passed successfully. This highlighted how automated tools can help maintain and improve security over time, while also showing the importance of carefully reviewing changes when applying fixes, and knowing what you are doing. 
